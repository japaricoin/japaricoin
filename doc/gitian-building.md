...
*Setup instructions for a Gitian build of Japaricoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Japaricoin
...
- [Building Japaricoin Core](#building-japaricoin-core)
...
Clone the git repositories for japaricoin and Gitian.
...
in the japaricoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Japaricoin Core
----------------

To build Japaricoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the japaricoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/japaricoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/japaricoin/japaricoin
...
For example:
```bash
URL=https://github.com/thrasher-/japaricoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit japaricoin=${COMMIT} --url japaricoin=${URL} ../japaricoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit japaricoin=${COMMIT} --url japaricoin=${URL} ../japaricoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit japaricoin=${COMMIT} --url japaricoin=${URL} ../japaricoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the japaricoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../japaricoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/japaricoin
SIGPATH=/some/root/path/japaricoin-detached-sigs

./bin/gbuild --url japaricoin=${BTCPATH},signature=${SIGPATH} ../japaricoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/japaricoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/japaricoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/japaricoin-osx-build.assert
```
...
[japaricoin/gitian.sigs.ltc](https://github.com/japaricoin/gitian.sigs.ltc/) repository, or if that's not possible create a pull
