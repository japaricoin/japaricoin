...
const char * const BITCOIN_CONF_FILENAME = "servalcoin.conf";
const char * const BITCOIN_PID_FILENAME = "servalcoin.pid";
...
#else
    const char* pszModule = "servalcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Servalcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Servalcoin";
#else
    // Unix
    return pathRet / ".servalcoin";
#endif
#endif
}
