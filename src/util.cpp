...
const char * const BITCOIN_CONF_FILENAME = "japaricoin.conf";
const char * const BITCOIN_PID_FILENAME = "japaricoin.pid";
...
#else
    const char* pszModule = "japaricoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Japaricoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Japaricoin";
#else
    // Unix
    return pathRet / ".japaricoin";
#endif
#endif
}
